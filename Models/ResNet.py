...
class BasicBlock(nn.Module):
    ...
    def __init__(self, args, in_planes, planes, stride=1, downsample=None):
        ...
        self.relu1 = nn.ReLU(inplace=True) #ReLuPCA(args, planes)

        self.conv2 = conv3x3(planes, planes)
        self.bn2 = nn.BatchNorm2d(planes)
        self.relu2 = nn.ReLU(inplace=True) #ReLuPCA(args, planes)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, args, inplanes, planes, stride=1, downsample=None):
        ...
        self.relu3 = nn.ReLU(inplace=True) #ReLuPCA(args, planes)
        self.downsample = downsample
        self.stride = stride
...
class ResNetImagenet(nn.Module):
    ...
    def __init__(self, block, layers, args, zero_init_residual=False):
        ...
        self.relu = nn.ReLU(inplace=True) #ReLuPCA(args, planes)
